...
export default function WebpToJpgPage() {
  const tool = TOOL_MAP.get("webp-to-jpg")!;
  ...
  const handleConvert = async () => {
    ...
    try {
      const jpgBlob = await ClientImageOps.convertFormat(file, "image/jpeg", 0.92);
      const url = URL.createObjectURL(jpgBlob);
      setResultSize(jpgBlob.size);
      setDownloadUrl(url);
    } catch (err: any) {
      setError(err?.message || "Failed to convert WebP to JPG.");
    } finally {
  ...
  return (
    <ToolLayout tool={tool}>
      {downloadUrl ? (
        <ResultCard
          filename={`${file?.name.replace(/\.webp$/i, "")}.jpg`}
          downloadUrl={downloadUrl}
          originalSizeBytes={file?.size}
          resultSizeBytes={resultSize}
          onReset={handleReset}
          title="Converted to JPG! 🎉"
          subtitle="Universally compatible JPG image ready for download."
        />
      ) : (
        <div className="w-full max-w-xl mx-auto space-y-6">
          {!file ? (
            <FileDropzone
              ...
              title="Drop WebP image to convert to JPG"
              subtitle="Convert WebP pictures into universally compatible JPGs"
            />
          ) : (
