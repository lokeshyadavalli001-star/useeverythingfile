...
export default function DeletePdfPagesPage() {
  const tool = TOOL_MAP.get("delete-pdf-pages")!;
  ...
  const handleDeleteMarked = async () => {
    ...
    if (selectedIndices.length === 0) {
      setError("Please select at least one page to delete by clicking on its thumbnail.");
      return;
    }
    ...
    try {
      const cleanedBytes = await ClientPDFOps.deletePages(pdfBuffer, selectedIndices);
      const blob = new Blob([cleanedBytes as any], { type: "application/pdf" });
      const url = URL.createObjectURL(blob);

      setResultSize(cleanedBytes.byteLength);
      setDownloadUrl(url);
    } catch (err: any) {
      setError(err?.message || "Failed to remove selected pages.");
    } finally {
  ...
  return (
    <ToolLayout tool={tool}>
      {downloadUrl ? (
        <ResultCard
          filename={`${file?.name.replace(/\.pdf$/i, "")}_cleaned.pdf`}
          downloadUrl={downloadUrl}
          originalSizeBytes={file?.size}
          resultSizeBytes={resultSize}
          onReset={handleReset}
          title="Pages Removed Successfully! 🎉"
          subtitle={`Removed ${selectedCount} page${selectedCount > 1 ? "s" : ""} from your document.`}
        />
      ) : (
          ...
          {!file || !pdfBuffer ? (
            <div className="max-w-xl mx-auto">
              <FileDropzone
                ...
                title="Drop PDF to delete pages"
                subtitle="Visually select and remove unwanted pages"
              />
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="sticky top-20 z-20 flex flex-wrap items-center justify-between gap-4 p-4 rounded-2xl bg-surface-900/95 backdrop-blur-md border border-surface-800 shadow-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-surface-400">
                      {pages.length} Total Pages • {selectedCount} Selected to Delete
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 disabled:opacity-40 text-white text-xs font-semibold shadow-lg shadow-red-600/30 transition-all active:scale-95"
                  >
                    {isProcessing ? (
                      <>
                      ...
                      <>
                        ...
                        <span>Delete {selectedCount} Page{selectedCount !== 1 ? "s" : ""}</span>
                      </>
                    )}
                  </button>
                </div>
              ...
              <div className="p-6 rounded-3xl bg-surface-950/60 border border-surface-800/80">
                <p className="text-xs text-surface-400 mb-4 font-medium">
                  Click on page thumbnails to select pages you want to remove:
                </p>
                <PDFPageGrid
                  pdfBuffer={pdfBuffer}
                  pages={pages}
